...
EVAL_JUDGE_LLM_MODEL = "gpt-4o-mini" #Changing the model to evaluate RAGAS metrics
#------
OLLAMA_MODEL = "gemma4:e4b"
MODEL_COMPARISON = [
    {"model": "Gemma4_RAG", "selection": "ollama", "rag": True},
    {"model": "Gemma4_only", "selection": "ollama", "rag": False},
    {"model": "GPT4o-mini_only", "selection": "openai", "rag": False},
]

DEFAULT_METRICS = ("context_precision", "context_recall", "faithfulness", "answer_relevancy", "semantic_similarity", "factual_correctness",)
...
RAG_METRIC_LABELS = {"context_precision", "context_recall", "faithfulness"}
...
def build_metrics(
    ...
    for metric_name in metric_names:
        ...
        #Adding new metrics for No-RAG evaluation
...
# For no-RAG samples, we generate an answer directly from the LLM without retrieval
def generate_no_rag_answer(question: str, selection: str) -> str:
    if selection == "ollama":
        llm = ChatOllama(model=OLLAMA_MODEL, num_ctx=2048) # Reduce memory usage, can removing if have more memory available
        print(f"No-RAG answer using Ollama: {OLLAMA_MODEL}")
    else:
        llm = ChatOpenAI(model=LLM_MODEL)
        print(f"No-RAG answer using OpenAI: {LLM_MODEL}")

    messages = [
        SystemMessage(content=(
            "You are a dental assistant. "
            "Answer the question as accurately as possible with 2-3 sentences only."
        )),
...
def build_sample_no_rag(row: EvalRow, selection: str) -> dict[str, Any]:
    answer = generate_no_rag_answer(row.question, selection)
    return {
        "row_index": row.row_index,
        "question": row.question,
        "reference": row.reference,
...
def build_metric_inputs(metric_name: str, traces: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for trace in traces:
        ...
        response = trace["assistant_response"] or trace.get("generated_answer") or ""
...
#         # Skip RAG-specific metrics if this model does not use RAG
#         if not model_info["rag"] and metric.name in RAG_METRIC_LABELS:
#             print(f"Skipping metric '{metric.name}' for RAG evaluation")
#             metric_scores[metric.name] = [{"value": None, "reason": "N/A"} for _ in traces]
#             continue
...
def score_metrics(*, traces, metrics, model_info):
    metric_scores = {}

    for metric in metrics:
        if not model_info["rag"] and metric.name in RAG_METRIC_LABELS:
            print(f"Skipping metric '{metric.name}' (no RAG)")
            metric_scores[metric.name] = [{"value": None, "reason": "N/A"} for _ in traces]
            continue
        ...
        for i, single_input in enumerate(metric_inputs):
            print(f"  [{metric.name}] scoring {i+1}/{len(metric_inputs)}...")
            while True:
                try:
                    results = metric.batch_score([single_input])
                    all_results.extend(results)
                    break  
                except Exception as e: # Catch all exceptions to handle rate limits and other transient issues
        ...
        metric_scores[metric.name] = [
            {"value": float(r.value) if r.value is not None else None, "reason": r.reason}
...
def main() -> None:
    ...
    for model_info in MODEL_COMPARISON:
        ...
        print(f"Evaluating model: {model_info['model']} (RAG: {model_info['rag']})")
        if model_info["rag"]:
            traces = [build_sample(row, vectorstore) for row in eval_rows]
        else:
            traces = [build_sample_no_rag(row, model_info["selection"]) for row in eval_rows]

        # Score metrics
        metric_scores = score_metrics(traces=traces, metrics=metrics, model_info=model_info)
        ...
        result_of_comparison.append(summary) #Save summary for comparison

    #------------------------------
    # Print comparison results
    print("\nModel Comparison Summary:")
    for model_info, summary in zip(MODEL_COMPARISON, result_of_comparison, strict=True):
        print(f"\nModel: {model_info['model']} (RAG: {model_info['rag']})")
        for metric_name in metric_columns:
            print(f"- {metric_name}: {summary['metric_means'][metric_name]:.4f}")
