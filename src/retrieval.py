    ...
    *,
    ...
    for ranked_documents in ranked_lists:
        for rank, document in enumerate(ranked_documents, start=1):
            ...
            fused_scores[chunk] = fused_scores.get(chunk, 0.0) + (1.0 / (k + rank))
    return fused_scores
...
def retrieve_nearest_chunk(
    ...
    rrf_scores_by_chunk = reciprocal_rank_fusion(
        [list(bm25_results), [document for document, _ in vector_results]]
    )

    candidate_documents = sorted(
        merged_documents.values(),
        key=lambda document: rrf_scores_by_chunk.get(document.page_content, 0.0),
        reverse=True,
    )[:RRF_CANDIDATE_K]
    ...
    rrf_score = rrf_scores_by_chunk.get(document.page_content, 0.0)
    accepted = rerank_score >= RERANK_ACCEPT_THRESHOLD
    ...
    return {
        ...
        "reason": None
        if accepted
        else "No sufficiently similar context found after reranking",
